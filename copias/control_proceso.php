...
if ((0 < $nump125 or
  ...
  if (
    ...
    $queryUpdate0 = "UPDATE cd SET id_tipo_oficina_fk_tipo_oficina = $dos, id_funcionario_fk_asignado = NULL WHERE id_cd = $uno ";
              ...
              <tbody>
                ...
                while ($row = $result->fetch_array(MYSQLI_ASSOC)) { ?>
                  <tr>
                    ...
                    if (000007 == $row['consecutivo_numero_cd']) {
                      echo '<span style="color:red">Expediente Prueba <br>' . $row['consecutivo_nomenclatura_cd'] . '-' . $row['consecutivo_numero_cd'] . '-' . $row['consecutivo_ano_cd'] . '</span>';
                    } else {
                      echo
                      '<a href="control_proceso_detalle&' . $row['id_cd'] . '.jsp">' .
                        $row['consecutivo_nomenclatura_cd'] . '-' . $row['consecutivo_numero_cd'] . '-' . $row['consecutivo_ano_cd'] .
                        '</a>';
                    }
                    echo '</td>';
