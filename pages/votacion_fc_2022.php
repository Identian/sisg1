...
$fecha_inicio = strtotime("2022-11-17 07:58:00");
$fecha_limite = strtotime("2022-11-17 16:58:00");
...
$query = sprintf("SELECT funcionario.id_funcionario, notaria.id_notaria,  cedula_funcionario, 
...
AND notaria.id_categoria_notaria=3 and fecha_fin is null and estado_posesion_notaria=1 and posesion_notaria.id_funcionario=".$valor." limit 1"); 
...
$insertSQL = sprintf("INSERT INTO votacion_fc_2022 
(nombre_votacion_fc_2022, id_candidato_votacion_fc_2022, id_funcionario, id_notaria, 
fecha_votacion_fc_2022, estado_votacion_fc_2022) VALUES (%s, %s, %s, %s, %s, %s)", 
...
$cuerpo .= 'La Superintendencia de Notariado y Registro informa que se ha registrado un voto para el fondo cuenta 2022.<br><br>';
...
$cuerpo2 .= 'La Superintendencia de Notariado y Registro informa que se ha registrado correctamente su voto a las elecciones para el fondo cuenta 2022.<br><br>';
...
$query = "SELECT * FROM candidato_votacion_fc2022 WHERE estado_candidato_votacion_fc2022=1 ";
$select = mysql_query($query, $conexion);
$row = mysql_fetch_assoc($select);
$totalRows = mysql_num_rows($select);
if (0<$totalRows){
...
$idv=$row['id_candidato_votacion_fc2022'];
...
<div class="col-md-6">
   ...
   <?php echo  $namenotaria2; ?>           
	...
	<?php		
$selectu = mysql_query("select count(id_votacion_fc_2022) as totale from votacion_fc_2022 where id_funcionario=".$idfun." and estado_votacion_fc_2022=1", $conexion);
...
<?php } else { echo 'No tiene acceso, solo para Notarios de 3 categoria'; }

	
	


	} else { 	echo 'Votación finalizada a las 17:00:00'; }
 ?>