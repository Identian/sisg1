...
//    $NomImagenR=$ubicacion."/".$NomImagen.'.'.$tipoArchivo[1];     
 ...
 //           echo $insertado;
	...
	$insertSQL = sprintf("INSERT INTO correo_cp (
      id_causante_cuota_parte, num_radicado, fecha_correo, 
	  nombre_correo_cp, name_file, tipo_name_file, correo_entidad) 
	  VALUES (%s, %s, now(), %s, %s, %s, %s)", 
      GetSQLValueString($id_causante_cuota_parte, "int"), 
	  GetSQLValueString($num_radicado, "text"),
	  GetSQLValueString($nombre_correo_cp, "text"),
	  GetSQLValueString($nombre_img, "text"),
	  GetSQLValueString($tipo_name_file, "int"),
	  GetSQLValueString($correo_entidad, "text")); 
      $Result = mysql_query($insertSQL, $conexion) or die(mysql_error());

	$insertSQL5 = sprintf("INSERT INTO expediente_cp (
...
error_reporting( E_ALL );
      ...
      $cuerpo .= "<br><br>";
	  $cuerpo .= '<br>Se adjunta archivo que contiene Cuenta de cobro del señor(a) '.$nombre_causante.'<br>';
...
if (0<$totalRows4) { 
	...
	$correo_entidad = $row4['correo_entidad'];
      ...
      <div class="container-fluid">
        ...
        <div id="navbar" class="navbar-collapse collapse">
            <ul class="nav navbar-nav">
			    <li><a href="causantes_cp&<?php echo $id_causante_cuota_parte; ?>.jsp"><b>CORREO ENTIDADES - CUOTA PARTE</b></a></li>
            </ul>
        </div>
...
<!-- 					   <button type="button" class="btn btn-success btn-xs nuevoc">Nuevo Contrato</button> -->
         		       <a href="" class="btn btn-sm btn-success" data-toggle="modal" data-target="#nmetafun"><span class="glyphicon glyphicon-plus-sign"></span> Nuevo Correo</a> 
					   </h4> 
					   <?php } ?>
                       <div class="box-body">
            ...
            <?php
               $query62 = sprintf("SELECT * 
	            FROM correo_cp   
                WHERE id_causante_cuota_parte = '$id_causante_cuota_parte'  
				AND tipo_name_file = 2 
				AND estado_correo_cp = 1 "); 
            ...
            ?>
          <tr>
             <td><?php echo $row62['id_correo_cp']; ?></td>
             <td><?php echo $row62['id_causante_cuota_parte']; ?></td>
             <td><?php echo utf8_encode($row62['correo_entidad']);?></td> 
		     ...
		     <td> 
			   <a href="correo_cp&<?php echo $row62['id_causante_cuota_parte']; ?>&<?php echo $row62['id_correo_cp']; ?>.jsp" class="confirmationdel" style="color:#ff0000;cursor: pointer" title="Borrar registro"  ><span class="glyphicon glyphicon-trash"></span></a>
			 </td>
			 <?php } ?>
         </tr>
          <?php } ?> 
...
<div class="modal-header"> 
  ...
  <h4 class="modal-title" id="myModalLabel"><b>NUEVO CORREO CUOTA PARTE</b><span style="font-weight: bold;"></span></h4>
</div> 
<div class="modal-body"> 
  <form action="" method="POST" name="form4311567534555"  enctype="multipart/form-data">
    ...
    <div class="form-group text-left"> 
        <label  class="control-label">Entidad:</label>   
        <input type="text" class="form-control" name="nombre_entidad" id="nombre_entidad" value="<?php echo $nombre_entidad; ?>" readonly = "readonly" >
    </div>

    <div class="form-group text-left"> 
        <label  class="control-label">Correo Entidad:</label>   
        <input type="text" class="form-control" name="correo_entidad" id="correo_entidad" value="<?php echo $correo_entidad; ?>" readonly = "readonly" >
    </div>
    ...
    <div class="form-group text-left"> 
        ...
        <input type="text" class="form-control" name="num_radicado" id="num_radicado" value="" required >
    </div>

    <div class="form-group text-left"> 
         <label  class="control-label"> ADJUNTAR CUENTA DE COBRO (en PDF):</label> 
	...
	<div class="modal-footer">
        ...
        <span class="glyphicon glyphicon-ok"></span>Enviar Correo</button></br>
	</div>
