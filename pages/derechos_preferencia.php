...
if ((isset($_POST["table"])) && ($_POST["table"] == "derecho_preferencia")) {
...
$directoryftp="files/derecho_preferencia/";


if (isset($_FILES['file']['name']) && ""!=$_FILES['file']['name']) {

$ruta_archivo = 'der_pref-'.$_SESSION['snr'].'-'.date("YmdGis");
...
$insertSQL = sprintf("INSERT INTO derecho_preferencia (nombre_derecho_preferencia, 
id_notaria,  fecha_publicacion, url, estado_derecho_preferencia) 
VALUES (%s, %s, now(), %s, %s)", 
GetSQLValueString($_POST["nombre_derecho_preferencia"], "text"), 
...
?>
  ...
  <div class="row">
   ...
   <div class="col-lg-3 col-xs-6">
    <div class="small-box bg-red">
            <div class="inner">
              <h3><?php echo existencia('derecho_preferencia'); ?></h3>
 ...
 <h3>
 <?php echo existencialimitada('derecho_preferencia', 'fecha_publicacion', $realdate) ?></h3>
...
$query4="SELECT * from derecho_preferencia, notaria, departamento where notaria.id_departamento=departamento.id_departamento and derecho_preferencia.id_notaria=notaria.id_notaria and estado_derecho_preferencia=1 ".$infop." ORDER BY fecha_publicacion desc ";
...
?>  
				...
				<tr>
				<?php
$id_res=$row['id_derecho_preferencia'];
echo '<td>'.$row['nombre_departamento'].'</td>';
echo '<td>'.$row['nombre_notaria'].'</td>';
echo '<td>'.$row['fecha_publicacion'].'</td>';
echo '<td>'.$row['nombre_derecho_preferencia'].'</td>';

echo '<td>';
echo ' <a href="files/derecho_preferencia/'.$row['url'].'" target="_blank"><img src="images/pdf.png"></a>';

	if (1==2) { //1==$_SESSION['rol'] or 0<$nump73
	echo ' <a style="color:#ff0000;cursor: pointer" title="Borrar" name="derecho_preferencia" id="'.$id_res.'" class="borrar_f"><span class="glyphicon glyphicon-trash"></span></a>';
	} else {}
echo '</td>';
?>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 style="font-weight: bold; text-align: center;" class="modal-title" id="myModalLabel">Nuevo derecho de preferencia</h4>
      </div>
...
<select class="form-control select2" name="id_notaria"  required>
...
<textarea class="form-control" name="nombre_derecho_preferencia"></textarea>
</div>
...
<input type="hidden" name="table" value="derecho_preferencia">
