...
$query = sprintf("SELECT * FROM boletin_orips"); 
$select = mysql_query($query, $conexion) or die(mysql_error());
...
<li><a ><i class="glyphicon glyphicon-home"></i> ORIP    <span class="pull-right"> <?php echo $depar; ?></span></a></li>
<li><a ><i class="glyphicon glyphicon-map-marker"></i> Departamento     <span class="pull-right"><?php echo $nombre_depa; ?></span></a></li>
...
<th>72% a Entidades Beneficiarias</th>
<th>28% a SNR</th>
</tr>
</thead>
<tbody>
...
$ct2=$c5+$c6+$c7+$c8+$c9+$c10; echo '<td>'.number_format((float)$ct2,2,",",".").'</td>';

echo '<td>'.$row['oc_vc'].'</td>';
echo '<td>'.$row['oc_vri'].'</td>';

$ctordinarios=$ct+$ct2; echo '<td>'.number_format((float)$ctordinarios,2,",",".").'</td>'; 
...
//LEY 55 72% 
$ctordinarios72=$ctordinarios*72/100; echo '<td>'.number_format((float)$ctordinarios72,2,",",".").'</td>'; 


//LEY 55 28% SNR
$ctordinarios28=$ctordinarios*28/100; echo '<td>'.number_format((float)$ctordinarios28,2,",",".").'</td>';
