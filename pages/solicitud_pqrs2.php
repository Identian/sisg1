...
echo $actualizado;
 ...
 if ((isset($_POST["tipo_req_tras"])) && (""!=$_POST["tipo_req_tras"])) {
if (0==$_POST["tipo_req_tras"]){
$motivo_movimiento=$_POST["motivo_requerimiento"];
$motivo_ciudadano=$_POST["respuesta_pre_ciudadano_nueva"];
$termin=5;
} else {
$motivo_movimiento=$_POST["motivo_traslado"];
$motivo_ciudadano=$_POST["respuesta_traslado_ciudadano"];
$termin=$_POST["terminos_notario"];
}
...
$insertSQL = sprintf("INSERT INTO requerir_pqrs (id_solicitud_pqrs, terminos_notaria, tipo_req_tras, nombre_requerir_pqrs, respuesta_pre_ciudadano, id_funcionario, id_tipo_oficina, id_vigilado, correo_oficina, estado_requerir_pqrs) VALUES 
(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", 
GetSQLValueString($id, "int"), 
GetSQLValueString($termin, "int"), 
GetSQLValueString($_POST['tipo_req_tras'], "int"), 
GetSQLValueString($motivo_movimiento, "text"), 
GetSQLValueString($motivo_ciudadano, "text"), 
GetSQLValueString($_SESSION['snr'], "int"), 
...
$selectbfk = mysql_query($querybfk, $conexion);
...
$tipo_req_tras_aprobar=$rowbfk['tipo_req_tras'];
} else {
...
$subject = 'Traslado de la Superintendencia de Notariado y Registro '.$radicado_salida.'';
$cuerpo = ''; 
$cuerpo .= "<div style='background:#f2f2f2;'><br><div style='margin: 0 auto;background:#fff;max-width:650px;width:100%;padding: 10px 10px 10px 10px;'><img src='https://servicios.supernotariado.gov.co/vista/img/snr-chat.jpg'><br>";
$cuerpo .= $nombre_requerir_pqrsk.""; 
$cuerpo .='<br>Puede ver la PQRSD del ciudadano en la URL: <a href="http://servicios.supernotariado.gov.co/pqrs/pdf/'.$radicado.'.pdf">http://servicios.supernotariado.gov.co/pqrs/pdf/'.$radicado.'.pdf</a>';
...
$tipo_req_tras_aprobar_ciudadano=$rowbfk['tipo_req_tras'];
...
if (0==$tipo_req_tras_aprobar_ciudadano) {
...
$updateSQL7797kt = sprintf("UPDATE solicitud_pqrs SET id_estado_solicitud=%s, dias_ampliacion=%s, fecha_inicio_ampliacion=now(), radicado_respuesta=%s, texto_ampliacion=%s WHERE id_solicitud_pqrs=%s and dias_ampliacion is null and estado_solicitud_pqrs=1",                   
                    GetSQLValueString(5, "int"), 					  
					  GetSQLValueString(15, "int"), 
					  GetSQLValueString($radicado_salida, "text"), 
                      GetSQLValueString($respuesta_pre_ciudadanok, "text"), 
...
$insertSQL33 = sprintf("INSERT INTO respuesta_pqrs (id_solicitud_pqrs, id_funcionario, nombre_respuesta_pqrs, fecha_respuesta, estado_respuesta_pqrs, radicado_entrada) VALUES (%s, %s, %s, now(), %s, %s)", 
GetSQLValueString($id, "int"), 
GetSQLValueString($_SESSION['snr'], "int"), 
GetSQLValueString($respuesta_pre_ciudadanok, "text"), 
GetSQLValueString(1, "int"),
GetSQLValueString($radicado_salida, "text"));
$Result = mysql_query($insertSQL33, $conexion) or die(mysql_error());
...
$subject = 'Respuesta PQRS de la Superintendencia de Notariado y Registro '.$radicado_salida.'';
$cuerpo = ''; 
$cuerpo .= "<div style='background:#f2f2f2;'><br><div style='margin: 0 auto;background:#fff;max-width:650px;width:100%;padding: 10px 10px 10px 10px;'><img src='https://servicios.supernotariado.gov.co/vista/img/snr-chat.jpg'><br>";
$cuerpo .= $respuesta_pre_ciudadanok.""; 
$cuerpo .= '<br><br>Superintendencia de Notariado y Registro<br>';
...
<select  class="form-control" name="tipo_req_tras" id="tipo_req_tras" required>
<option selected></option>
<option value="0">Requerimiento</option>
...
<label  class="control-label">REQUERIMIENTO PARA EL VIGILADO:</label> 
...
<div id="divtras" style="display:none;">

<label  class="control-label">TRASLADO PARA EL VIGILADO:</label> 

Terminos: <input value="<?php echo $dias_terminos; ?>" type="text" name="terminos_notario">
<textarea spellcheck="true" lang="es"  class="form-control" name="motivo_traslado" id="texto_trasladar">
<?php
$fec9= $diam.' de '.mese($mesm).' de '.$anom;
echo 'Bogotá, '.$fec9.'';
?>
...
<br><br>
<p align="justify" style="text-align: justify; text-justify: inter-word;">
Mediante el radicado de la referencia se traslada una (<?php echo $categoria; ?>) suscrita por el señor/a <?php echo $nombre; ?><br>
...
<div class="form-group text-left"> 
<div id="divreqciu" style="display:none;">
<label  class="control-label">MENSAJE DEL REQUERIMIENTO AL CIUDADANO:</label> 
...
<div id="divtrasciu" style="display:none;">
<label  class="control-label">MENSAJE DEL TRASLADO AL CIUDADANO:</label> 
<textarea spellcheck="true" lang="es"  class="form-control" name="respuesta_traslado_ciudadano" id="texto_trasladar_ciudadano">
...
Referencia: Traslado <?php echo $radicado; ?> de fecha <?php 
echo date("Y-m-d",strtotime($fecha_radicado.""));?>
<br><br>
Respetado/a señor/a, reciba un atento saludo.
<br><br>
<p align="justify" style="text-align: justify; text-justify: inter-word;">
En respuesta al radicado de la referencia, se informa que dicha (PETICIÓN/COMUNICACIÓN/QUEJA) fue trasladada a la Notaría, 
</p>
...
$resultll = mysql_query($queryll); 
  ...
  $updateSQLm = sprintf("UPDATE ciudadano SET idcorreocontactoiris=%s WHERE id_ciudadano=%s and estado_ciudadano=1",   
...
$infotiporeq=$rowbf['tipo_req_tras'];
?>
<hr>
<div>
<h4><b>
<?php if (0==$infotiporeq) {
	echo 'Requerimiento';
} else {
	echo 'Traslado';
}
...
<?php
 if (0==$infotiporeq) {
	echo '<input type="hidden" name="tipo_req_tras_aprobar" value="0">';
} else {
	echo '<input type="hidden" name="tipo_req_tras_aprobar" value="1">';
}
?>
<button type="submit" class="btn btn-success" style="width:100%">
...
<h4><b>
<?php if (0==$infotiporeq) {
	echo 'Respuesta preliminar al ciudadano sobre el requerimiento';
} else {
	echo 'Respuesta de fondo al ciudadano por el traslado';
}
?>
</b></h4>
...
<?php
 if (0==$infotiporeq) {
	echo '<input type="hidden" name="tipo_req_tras_aprobar_ciudadano" value="0">';
} else {
	echo '<input type="hidden" name="tipo_req_tras_aprobar_ciudadano" value="1">';
}
?>
<button type="submit" class="btn btn-success" style="width:100%">
<span class="glyphicon glyphicon-envelope"></span>  Aprobar y enviar</button>
...
<?php
$queryb = sprintf("SELECT * FROM requerir_pqrs where id_solicitud_pqrs=".$id." and tipo_req_tras=0 and estado_requerir_pqrs=1"); 
...
$result8 = $mysqli->query($query48);
	...
	if (5!=$estado_solicitud_pqrs && (1==$_SESSION['rol'] or ((1==$_SESSION['snr_lider_pqrs'] or 1==$_SESSION['snr_grupo_cargo']) and 24==$_SESSION['snr_grupo_area'])))
		{	echo '<a style="color:#ff0000;cursor: pointer" title="Borrar" name="clasificacion_pqrs" id="'.$row9['id_clasificacion_pqrs'].'" class="borrar_f"><span class="glyphicon glyphicon-trash"></span></a> '; } else {}
			...
			$dias_terminos=dias_terminos($row9['id_clase_oac']);
...
if (5!=$estado_solicitud_pqrs && 2<$dias_terminos && (1==$_SESSION['rol'] or (0<$sigrupo and 1==$_SESSION['snr_grupo_cargo'] && 1==$_SESSION['snr_tipo_oficina']))) { ?>
...
<div class="box box-warning direct-chat direct-chat-warning" >
    <div class="box-header with-border">
<h3 class="box-title">
<?php if ((24==$_SESSION['snr_grupo_area'] or 1==$_SESSION['rol']) && 2<$dias_terminos && 5!=$estado_solicitud_pqrs) { ?>
...
<h3 class="box-title">
<?php 
$querybn = sprintf("SELECT count(id_requerir_pqrs) as totreq FROM requerir_pqrs where id_solicitud_pqrs=".$id." and estado_requerir_pqrs=1"); 
$selectbn = mysql_query($querybn, $conexion) or die(mysql_error());
$rowbn = mysql_fetch_assoc($selectbn);

if (0<$rowbn['totreq']) { 
echo 'Movimiento proyectado';
} else { }
...
<span class="glyphicon glyphicon-inbox"></span>  MOVIMIENTO</a>
<?php } else { echo 'Movimiento'; }
 ?>
</h3>
                  <div class="box-tools pull-right">       
                    <button type="button" class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i>
...
if (0<$totalRowsb) {
	...
	if (0==$rowb['tipo_req_tras']) { echo 'Requerido / '; } else { echo 'Traslado / ';}
...
$query48 = sprintf("SELECT * FROM solicitud_pqrs, ciudadano where ciudadano.id_ciudadano=".$id_ciudadano."  and solicitud_pqrs.id_ciudadano=ciudadano.id_ciudadano and estado_solicitud_pqrs=1  order by solicitud_pqrs.id_solicitud_pqrs desc limit 50"); 
