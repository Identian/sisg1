...
if ((isset($_POST["table"])) && ($_POST["table"] == "instruccion_admin") && (1==$_SESSION['rol'] or 0<$nump73)) {
...
$directoryftp="files/content/memos/";

if (isset($_FILES['file']['name']) && ""!=$_FILES['file']['name']) {

$ruta_archivo = 'memo-'.$_POST["numero_archivo"].'-'.date("YmdGis");
...
numero_archivo, id_tipo_oficina, 
codigo_oficina, nombre_archivo, fecha_publicacion, tipo, carpeta, 
url, id_normatividad, codificado, estado_archivo) VALUES (%s, %s, %s, %s, now(), %s, %s, %s, %s, %s, %s)", 
GetSQLValueString($_POST["numero_archivo"], "int"), 
GetSQLValueString($_POST["id_tipo_oficina"], "int"), 
GetSQLValueString($_POST["codigo_oficina"], "int"), 
GetSQLValueString($_POST["nombre_archivo"], "text"), 
GetSQLValueString('memos', "text"),
GetSQLValueString('content/memos', "text"),
  ...
  <div class="row">
   ...
   <div class="col-lg-3 col-xs-6">
    <div class="small-box bg-red">
            <div class="inner">
              <h3><?php echo existenciaunica('archivo', 'id_normatividad', 7); ?></h3>
...
<select class="form-control" name="campo" required>
 ...
 <option value="num_instruccion_admin">Número</option>
<option value="nombre_instruccion_admin">Asunto</option>
		  </select>
</div>
...
$query4="SELECT * from archivo where estado_archivo=1 ".$infop." and id_normatividad=6 ORDER BY fecha_publicacion desc  "; //LIMIT 500 OFFSET ".$pagina."
...
if (isset($row['id_tipo_oficina']) && 1==$row['id_tipo_oficina']) {
echo 'Nivel Central, ';
echo quees('area', $row['codigo_oficina']);
} else if (isset($row['id_tipo_oficina']) && 2==$row['id_tipo_oficina']) {
	echo 'ORIP, ';
echo quees('oficina_registro', $row['codigo_oficina']);
	} else if (isset($row['id_tipo_oficina']) && 3==$row['id_tipo_oficina']) {
echo 'Notaria, ';
echo quees('notaria', $row['codigo_oficina']);
		} else if (isset($row['id_tipo_oficina']) && 4==$row['id_tipo_oficina']) {
	echo 'Curaduria, ';
...
<?php
$query = sprintf("SELECT id_tipo_oficina, nombre_tipo_oficina FROM tipo_oficina where id_tipo_oficina in (1, 2, 3, 4) and estado_tipo_oficina=1 order by id_tipo_oficina"); 
