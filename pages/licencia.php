...
fecha_ejecutoria=%s, n_acto_administrativo=%s, certificado_ocupacion=%s, autorizacion_ocupacion=%s, observacion_licencia=%s, 
id_estado_lic_curaduria=%s, 
...
<div class="form-group text-left"> 
<label  class="control-label">NÚMERO DE ACTO ADMINISTRATIVO:</label>   
...
/*
 if (0==$estado or 0<$nump21 or 1==$_SESSION['rol']) {
echo '<a id="'.$row_update['nombre_licencia_curaduria'].'" name="'.$row_update['id_licencia_curaduria'].'" class="ventana1" data-toggle="modal" data-target="#popupcostolicencia" href="" title="Añadir"> <button type="button" class="btn btn-xs btn-success"><span class="glyphicon glyphicon-plus-sign"></span> Agregar</button></a>';
 } else {}
 */
?>
</div>					  
...
<?php
/*
$query = sprintf("SELECT * FROM costo_licencia  where estado_costo_licencia=1 and id_licencia_curaduria='$id'  
order by id_costo_licencia"); 
...
do {
	...
	echo 'Cargo Fijo: '.$row['cargo_fijo'].'';
		 if (0==$estado or 0<$nump21 or 1==$_SESSION['rol']) {
	echo ' <a style="color:#ff0000;cursor: pointer" title="Borrar" name="costo_licencia" id="'.$row['id_costo_licencia'].'" class="borrar_f"><span class="glyphicon glyphicon-trash"></span></a>';
		 } else {}
	
	echo '<br />'; 
		echo 'Cargo variable: '.$row['cargo_variable'].'<hr>';
