<?php
$nump88=privilegios(88,$_SESSION['snr']);

if (1==$_SESSION['rol'] or 3>$_SESSION['snr_tipo_oficina']) {


if ((isset($_POST["sentido"])) && (""!=$_POST["sentido"]) && (0<$nump88 or 1==$_SESSION['rol'])) {
...
$directoryftp="filesnr/sentencias/";
...
$insertSQL = sprintf("INSERT INTO sentencia (demandante, sentido, ano, fecha_publicacion,  
nombre_sentencia, url, estado_sentencia) VALUES (%s, %s, %s, now(), %s, %s, %s)", 
GetSQLValueString($_POST["demandante"], "text"), 
GetSQLValueString($_POST["sentido"], "text"), 
GetSQLValueString($_POST["ano"], "int"), 
GetSQLValueString($_POST["nombre_sentencia"], "text"), 
...
?>
  ...
  <div class="row">
   ...
   <div class="col-lg-3 col-xs-6">
    <div class="small-box bg-red">
            <div class="inner">
              <h3><?php echo existencia('sentencia'); ?></h3>

              <p>Cantidad de sentencias</p>
            </div>
            <div class="icon">
 ...
 <?php echo existencialimitada('concepto', 'fecha_publicacion', $realdate) ?></h3>
              ...
              <p>Sentencias registradas hoy</p>
            </div>
            <div class="icon">
...
<div class="box-header with-border">
  ...
  <div class="col-md-4">
<?php  if (1==$_SESSION['rol'] or 0<$nump88) { ?>
...
$query4="SELECT * from sentencia where estado_sentencia=1 ".$infop." ORDER BY fecha_publicacion desc ";
...
?>  
				...
				<tr>
				<?php
$id_res=$row['id_sentencia'];
echo '<td title="'.$row['fecha_publicacion'].'">'.$row['demandante'].'</td>';
echo '<td>'.$row['sentido'].'</td>';
echo '<td>'.$row['ano'].'</td>';
echo '<td>'.$row['nombre_sentencia'].'</td>';

echo '<td>';

echo '<a target="_blank" href="filesnr/sentencias/'.$row['url'].'"><img src="images/pdf.png"></a>';

	if (1==$_SESSION['rol'] or 0<$nump88) { 
	echo ' <a style="color:#ff0000;cursor: pointer" title="Borrar" name="sentencia" id="'.$id_res.'" class="borrar_f"><span class="glyphicon glyphicon-trash"></span></a>';
	} else {}
echo '</td>';
?>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 style="font-weight: bold; text-align: center;" class="modal-title" id="myModalLabel">Nueva sentencia</h4>
      </div>
...
<label  class="control-label"><span style="color:#ff0000;">*</span> DEMANDANTE:</label> 
<input type="text" class="form-control" name="demandante"  required>
</div>
<div class="form-group text-left"> 
<label  class="control-label"><span style="color:#ff0000;">*</span> SENTIDO:</label> 
...
<textarea class="form-control" name="nombre_sentencia" required></textarea>
</div>
