...
$query = sprintf("SELECT correo_funcionario  FROM asignacion_pqrs_funcionario, funcionario where  asignacion_pqrs_funcionario.id_funcionario=funcionario.id_funcionario and id_solicitud_pqrs=12899 and estado_asignacion_pqrs_funcionario=1"); 
...
$id_tipologiapqrs_notaria=$_POST["id_tipologiapqrs_notaria"];
$insertSQLi = sprintf("INSERT INTO clasificacionpqrs_dvcn (id_solicitud_pqrs, id_tipologiapqrs_notaria, id_tipo_oficina, id_vigilado, fecha_clasificacionpqrs_dvcn, estado_clasificacionpqrs_dvcn) VALUES (%s, %s, %s, %s, now(), %s)", 
GetSQLValueString($id, "int"), 
GetSQLValueString($id_tipologiapqrs_notaria, "int"), 
...
echo $actualizado;
 ...
 if ((isset($_POST["respuesta_control_ciudadano_nueva"])) && ($_POST["respuesta_control_ciudadano_nueva"] != "")) { 

$id_respuesta= $_POST["id_res"];
$textopqrs2= limpiar($_POST["respuesta_control_ciudadano_nueva"]);
...
estado_conocimiento_pqrs) VALUES (%s, %s, %s, %s, now(), %s, %s)", 
GetSQLValueString($id, "int"), 
GetSQLValueString($_SESSION['snr'], "int"), 
...
$cuerpo = ''; 
$cuerpo .= "<div style='background:#f2f2f2;'><br><div style='margin: 0 auto;background:#fff;max-width:650px;width:100%;padding: 10px 10px 10px 10px;'><img src='https://servicios.supernotariado.gov.co/vista/img/snr-chat.jpg'>";
$cuerpo .= "Vicky te informa que la Oficina de Atención al Ciudadano pone en su conocimiento la PQRSD ".$radicado_pqrsf."<br>";
...
$cuerpo .= '<br>';
$infoacuse1=base64_encode($correo_jefe);
$bodytag = str_replace("=", "", $infoacuse1);
$infoacuse=$bodytag.'&'.$radicado_pqrsf;
...
//if (1==$_SESSION['rol'] or ((24 == $_SESSION['snr_grupo_area'] or 41==$_SESSION['snr_grupo_area']) and (1==$_SESSION['snr_grupo_cargo'] or 1==$_SESSION['snr_lider_pqrs']))) {
		$infoval='';
		$infoval2='';
/*	} else {  
$id_func=$_SESSION['snr'];
$infoval=', asignacion_pqrs_funcionario ';
$infoval2=' and asignacion_pqrs_funcionario.id_solicitud_pqrs='.$id.' and   solicitud_pqrs.id_solicitud_pqrs=asignacion_pqrs_funcionario.id_solicitud_pqrs and estado_asignacion_pqrs_funcionario=1 and asignacion_pqrs_funcionario.id_funcionario='.$id_func;
	}
	*/
	...
	/*
$query48 = sprintf("SELECT solicitud_pqrs.id_solicitud_pqrs FROM solicitud_pqrs, canal_pqrs, categoria_pqrs, ciudadano, estado_solicitud ".$infoval." where solicitud_pqrs.id_estado_solicitud=estado_solicitud.id_estado_solicitud and solicitud_pqrs.id_ciudadano=ciudadano.id_ciudadano and solicitud_pqrs.id_categoria_pqrs=categoria_pqrs.id_categoria_pqrs and solicitud_pqrs.id_canal_pqrs=canal_pqrs.id_canal_pqrs and estado_solicitud_pqrs=1 ".$infoval2." order by solicitud_pqrs.id_solicitud_pqrs asc"); 
$selectls8 = mysql_query($query48, $conexion) or die(mysql_error());
$rowls8 = mysql_fetch_assoc($selectls8);
...
$updateSQL7797k = sprintf("UPDATE solicitud_pqrs SET id_estado_solicitud=%s WHERE id_solicitud_pqrs=%s and estado_solicitud_pqrs=1",                  
					   ...
					   GetSQLValueString($id, "int"));
$Result17797k = mysql_query($updateSQL7797k, $conexion) or die(mysql_error());
...
/*
$subject = 'Requerimiento de la Superintendencia de Notariado y Registro';
$cuerpo .= $radicado_salida."\n\n"; 
$cuerpo .= $nombre_requerir_pqrsk."\n\n\n\n"; 
$cuerpo .='<a href="http://servicios.supernotariado.gov.co/pqrs/pdf/'.$radicado.'.pdf"><button type="button" style="background:#B40404;color:#ffffff;height:30px;width:300px;text-align:center;border-radius: 10px;cursor:pointer;"> Ver PQRS del ciudadano </button>  http://servicios.supernotariado.gov.co/pqrs/pdf/'.$radicado.'.pdf</a>';
$cuerpo .='<a href="https://sisg.supernotariado.gov.co/requerimiento_respuesta&'.$id.'.jsp"><button type="button" style="background:#B40404;color:#ffffff;height:30px;width:300px;text-align:center;border-radius: 10px;cursor:pointer;"> Dar respuesta </button> https://sisg.supernotariado.gov.co/requerimiento_respuesta&'.$id.'.jsp </a>';
$cuerpo .= "<br><br>"; 
$cabeceras .= 'From: Supernotariado<notificadorD@supernotariado.gov.co>' . "\r\n";
$cabeceras .= 'Bcc: sisg@supernotariado.gov.co' . "\r\n";
$cabeceras .= "MIME-Version: 1.0\r\n";
...
$cuerpo .= $nombre_requerir_pqrsk.""; 
$cuerpo .='<br>Puede ver la PQRSD del ciudadano en la URL: <a href="http://servicios.supernotariado.gov.co/pqrs/pdf/'.$radicado.'.pdf">http://servicios.supernotariado.gov.co/pqrs/pdf/'.$radicado.'.pdf</a>';
...
$updateSQL7797 = sprintf("UPDATE requerir_pqrs SET radicado_ciudadano=%s WHERE  id_solicitud_pqrs=%s and estado_requerir_pqrs=1",                  
					   GetSQLValueString($radicado_salida, "text"),
					   GetSQLValueString($id, "int"));
...
/*
$subjectc = 'Respuesta preliminar de la Superintendencia de Notariado y Registro';
$cuerpoc = '';
$cuerpoc .= $radicado_salida."\n\n"; 
$cuerpoc .= $respuesta_pre_ciudadanok."\n\n"; 
$cuerpoc .= "\n\n\n\n\n\n"; 
$cabecerasc ='';
$cabecerasc .= 'From: Supernotariado<notificadorD@supernotariado.gov.co>' . "\r\n";
$cabecerasc .= 'Bcc: sisg@supernotariado.gov.co' . "\r\n";
$cabecerasc .= "MIME-Version: 1.0\r\n";
$cabecerasc .= "Content-Type: text/html; charset=UTF-8\r\n";
mail($emailuc,$subjectc,$cuerpoc,$cabecerasc);
*/
...
$insertSQL5 = sprintf("INSERT INTO documento_pqrs (idcorrespondencia, id_ciudadano, id_funcionario, nombre_documento_pqrs, id_solicitud_pqrs, id_clase_documento, fecha_subida, url_documento, extension, hash_documento, estado_documento_pqrs) VALUES (%s, %s, %s, %s, %s, %s, now(), %s, %s, %s, %s)", 
GetSQLValueString(1, "int"), 
...
$consultaghyyoor = mysql_query("SELECT count(id_respuesta_pqrs) as tres FROM respuesta_pqrs where id_solicitud_pqrs=".$id." and estado_respuesta_pqrs=1", $conexion) or die(mysql_error());
...
GetSQLValueString($_POST["id_categoria_oac"], "int"),
 GetSQLValueString($_POST["id_clase_oac"], "int"), 
...
<br>
De manera atenta nos permitimos informarle que su (petición, queja o reclamo, según el caso) con radicado No. ".$radicado." ha sido direccionado  el dia ".$realdate.",  a la Oficina de Registro de Instrumentos Públicos ".$nombre_oripzxx.", quien es la Oficina competente para dar respuesta a su (petición, queja o reclamo, según el caso).
<br>
...
<textarea class="form-control" name="nombre_conocimiento_pqrs" ></textarea>
</div>
...
<div class="modal-header"> 
<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Cerrar</span></button>
<h4 class="modal-title" id="myModalLabel"><b>CLASIFICACIÓN INTERNA - DVCN:</b> <span class="licenciac" ></span></h4>
</div> 
<div id="nuevaAventura" class="modal-body"> 
...
<div class="form-group text-left"> 
<label  class="control-label"><span style="color:#ff0000;">*</span>OFICINA:</label> 
<select class="form-control" id="ver_ofi2" name="id_vigilado" required>
</SELECT>
</div>
<div class="form-group text-left"> 
...
<div class="modal-header"> 
<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Cerrar</span></button>
<h4 class="modal-title" id="myModalLabel"><b>REQUERIR:</b> <span class="licenciac" ></span></h4>
</div> 
<div id="nuevaAventura" class="modal-body"> 
<form action="" method="POST" name="form1FGDG">
...
Referencia: Requerimiento con Radicado <?php echo $radicado; ?> de fecha <?php 
...
Mediante el radicado de la referencia fue recibida una (<?php echo $categoria; ?>) suscrita por el señor/a <?php echo $nombre; ?>, relacionada con la presunta afectación de la adecuada prestación del Servicio Público Notarial, cuyo texto puede consultar haciendo clic en el botón que encontrará en la parte inferior del presente correo.<br>
<br><br>
Con la finalidad de dar una respuesta de fondo al peticionario, le solicito muy respetuosamente se pronuncie sobre los hechos relacionados en la referida comunicación y, aporte los documentos y/o soportes que respalden sus consideraciones. La información solicitada debe ser proporcionada en un término ÚNICO de cinco (05) días hábiles, los cuales se contarán a partir del día siguiente al recibo de este requerimiento, la respuesta deberá ser tramitada ingresando al botón respectivo que encontrará en la parte inferior del presente correo. <br>

<u>Los soportes o anexos que excedan de 10 Mb deberán ser enviados a esta Dirección a través de correo certificado.</u>
<br><br>
Para todos los efectos legales, se recuerda la obligación de dar oportuna contestación a este requerimiento, teniendo cuenta lo preceptuado en el numeral 3° del Artículo 55 de la Ley 734 de 2002. 
</p><br><br>
Atentamente,
<br><br>
<br>
SUMAYA CHEJNE DUARTE<br>
...
Referencia: Respuesta del Requerimiento <?php echo $radicado; ?> de fecha <?php 
echo date("Y-m-d",strtotime($fecha_radicado.""));?>
<br><br>
Respetado/a señor/a, reciba un atento saludo.
<br><br>
<p align="justify" style="text-align: justify; text-justify: inter-word;">
En respuesta al radicado de la referencia, se informa que dicha (PETICIÓN/COMUNICACIÓN/QUEJA) fue remitida a la Notaría, con el fin de que rinda las explicaciones del caso ante esta Delegada.
<br><br>
Una vez sea obtenido el respectivo pronunciamiento por parte de la Notaría, le estaremos brindando una respuesta de fondo a su solicitud, dentro del término máximo previsto en la ley.
<br><br>
Cabe resaltar que la Delegada para el Notariado está presta a recibir sus solicitudes e inconformidades y tomar las acciones pertinentes para mejorar la prestación del servicio público notarial.
</p>
...
$resultll = mysql_query($queryll); 
  ...
  $updateSQLm = sprintf("UPDATE ciudadano SET idcorreocontactoiris=%s WHERE id_ciudadano=%s and estado_ciudadano=1  and id_ciudadano!=21373",   
					 ...
					 GetSQLValueString($id_ciudadano, "int"));
  $Result1m = mysql_query($updateSQLm, $conexion) ;
...
echo '<b>Radicado:</b> ';
echo ''.$radicado.'<br>';
echo '<a href="http://servicios.supernotariado.gov.co/pqrs/pdf/'.$radicado.'.pdf"><img src="images/pdf.png"> Constancia</a>';
...
do {
      ...
      echo '<a href="files/'.$row['url_documento'].'" target="_black"><img src="images/pdf.png"> '.$row['nombre_documento_pqrs'].'</a>';
       echo '<br>';
      echo 'Subido: '.$row['fecha_subida'].' - HASH: '.$row['hash_documento'].'<br><br>';
...
<div class="col-md-3"> 
<?php if (1==$_SESSION['snr_grupo_cargo'] or 1==$_SESSION['rol']) { ?>
...
<hr>
<?php if (isset($_GET['e']) and 2==$_GET['e']) { ?>
...
<hr>
<?php if (isset($_GET['e']) and 3==$_GET['e']) { ?>
...
echo '<br>';
	...
	echo '<h3>AÚN NO EXISTE RESPUESTA DESDE LA CUENTA DEL NOTARIO</h3>';
...
echo '<a title="'.$row['fecha_subida'].' - HASH: '.$row['hash_documento'].'" href="files/'.$row['url_documento'].'" target="_blank"><img src="images/pdf.png"> ';
...
$consulta = mysql_query("SELECT id_respuesta_pqrs, nombre_respuesta_pqrs, id_funcionario, id_funcionario_reviso FROM respuesta_pqrs WHERE id_solicitud_pqrs='$id' and estado_respuesta_pqrs=1 limit 1", $conexion) or die(mysql_error());
$row1 = mysql_fetch_assoc($consulta);
$nn = mysql_num_rows($consulta);
if (0<$nn) {
 ?>
 <?php if (isset($_GET['e']) and 1==$_GET['e']) { } else { ?>
<div class="row" >
<div class="col-md-12"> 
<?php if (5==$estado_solicitud_pqrs) {
...
if (5==$estado_solicitud_pqrs ) { ?>
<a href="http://servicios.supernotariado.gov.co/pqrs/pdf/<?php echo $row4['radicado_respuesta']; ?>.pdf" download="<?php echo $row4['radicado_respuesta']; ?>.pdf" class="btn btn-danger" style="width:100%" >
...
$id_clase_oac=$row9['id_clase_oac'];
$actualizar44 = mysql_query("SELECT * FROM clase_oac WHERE id_clase_oac='$id_clase_oac' limit 1", $conexion) or die(mysql_error());
$row144 = mysql_fetch_assoc($actualizar44);
//$valt=intval($row144['terminos_dias']);
$valt=intval($row144['terminos_ampliados']);			
if (1<$valt and 5!=$estado_solicitud_pqrs){
	
	if (6==$estado_solicitud_pqrs or 7==$estado_solicitud_pqrs) {  
	echo '<br>Los terminos se enuncian en la sección del requerimiento.';
	} else {
	
	echo '(Terminos: '.$valt.' dias habiles)<br>';

	
	$fechavence=fechahabil($fecha_radicado,$valt);
...
$insertSQL3355= sprintf("INSERT INTO ampliacion_pqrs (id_solicitud_pqrs, ampliacion_dias, fecha_ampliacion, 
nombre_ampliacion_pqrs, estado_ampliacion_pqrs) VALUES (%s, %s, now(), %s, %s)", 
GetSQLValueString($id, "int"), 
GetSQLValueString($dias_ampliados, "int"), 
GetSQLValueString($ampliacion_terminos, "text"), 
GetSQLValueString(1, "int"));
$Result3355 = mysql_query($insertSQL3355, $conexion);
...
<div class="modal-header"> 
<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Cerrar</span></button>
<h4 class="modal-title" id="myModalLabel"><b>AMPLIACIÓN DE TERMINOS:</b> <span class="licenciac" ></span></h4>
</div> 
<div id="nuevaAventura" class="modal-body"> 
<form action="" method="POST" name="form1F5454543245345345GDG">

<?php
if ((0<$sigrupo && 1==$_SESSION['snr_grupo_cargo']) or 1==$_SESSION['rol']) {
?>
<div class="form-group text-left"> 
<label  class="control-label">Número de dias que quiere ampliar:</label>
...
$querybn = sprintf("SELECT count(id_ampliacion_pqrs) as totreqc FROM ampliacion_pqrs where id_solicitud_pqrs=".$id." and estado_ampliacion_pqrs=1"); 
...
<?php } else { echo ' AMPLIACIÓN DE TERMINOS'; }
 ...
 <div class="box-body" >
				<div  class="modal-body" style="max-height:250px;">
<?php
$queryt = sprintf("SELECT * FROM ampliacion_pqrs where id_solicitud_pqrs=".$id." and estado_ampliacion_pqrs=1"); 
$selectt = mysql_query($queryt, $conexion);
$rowt = mysql_fetch_assoc($selectt);
$totalRowst = mysql_num_rows($selectt);
if (0<$totalRowst){
do {
	echo 'Fecha de ampliación: '.$rowt['fecha_ampliacion'].'<br>';
echo 'Dias ampliados: '.$rowt['ampliacion_dias'].'<br>';
echo 'Nueva fecha: ';
	$nuevafechavence=fechahabil($rowt['fecha_ampliacion'],$rowt['ampliacion_dias']);
...
<div class="box box-warning direct-chat direct-chat-warning" >
    <div class="box-header with-border">
<h3 class="box-title">
<?php if ((24==$_SESSION['snr_grupo_area'] or 1==$_SESSION['rol']) && 5>$estado_solicitud_pqrs) { ?>
...
$queryb = sprintf("SELECT * FROM conocimiento_pqrs, area where conocimiento_pqrs.id_area=area.id_area and id_solicitud_pqrs=".$id." and estado_conocimiento_pqrs=1"); 
$selectb = mysql_query($queryb, $conexion) or die(mysql_error());
$rowb = mysql_fetch_assoc($selectb);
$totalRowsb = mysql_num_rows($selectb);
...
do {
	...
	echo '<span style="color:#777;font-size:12px;">'.$rowb['fecha_conocimiento_pqrs'].'</span><br>Para: ';
	
	echo $rowb['nombre_area'].'<br>De: ';
...
<?php
$query48 = sprintf("SELECT lider_pqrs, nombre_funcionario, fecha_asignacion_funcionario, motivo_asignacion, nombre_cargo, id_funcionario_asigna, id_grupo_area, id_asignacion_pqrs_funcionario  FROM asignacion_pqrs_funcionario, funcionario, cargo where funcionario.id_cargo=cargo.id_cargo and asignacion_pqrs_funcionario.id_funcionario=funcionario.id_funcionario and  id_solicitud_pqrs=".$id." and estado_asignacion_pqrs_funcionario=1 order by id_asignacion_pqrs_funcionario desc"); 
$result8 = $mysqli->query($query48);
...
if (5==$estado_solicitud_pqrs){ } else {
if ((((24 == $_SESSION['snr_grupo_area'] OR 45 == $_SESSION['snr_grupo_area']) and (1==$_SESSION['snr_lider_pqrs'] or 1==$_SESSION['snr_grupo_cargo']))  and (5!=$estado_solicitud_pqrs and (24==$row9['id_grupo_area'] OR 45==$row9['id_grupo_area']))) or (1==$_SESSION['rol'])) {
...
REVISIÓN DEL ÁREA
 ...
 <div class="box-body" >
				...
				<?php 
if ((0<$sigrupo && 45==$_SESSION['snr_grupo_area'] && (1==$_SESSION['snr_grupo_cargo'] or 1==$_SESSION['snr_lider_pqrs']) ) or 1==$_SESSION['rol']) { 
...
<h3 class="box-title">
<?php 
$querybn = sprintf("SELECT count(id_clasificacionpqrs_dvcn) as totreqf FROM clasificacionpqrs_dvcn where id_solicitud_pqrs=".$id." and estado_clasificacionpqrs_dvcn=1 limit 1"); 
$selectbn = mysql_query($querybn, $conexion) or die(mysql_error());
$rowbn = mysql_fetch_assoc($selectbn);

if (0<$rowbn['totreqf']) { 
echo ' CLASIFICACIÓN - DVCN';
} else {
...
<span class="glyphicon glyphicon-inbox"></span> CLASIFICACIÓN - DVCN</a>
<?php } else { echo ' CLASIFICACIÓN - DVCN'; }
}  ?>
</h3>
                  <div class="box-tools pull-right">       
                    <button type="button" class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i>
...
$queryb = sprintf("SELECT * FROM clasificacionpqrs_dvcn where clasificacionpqrs_dvcn.id_solicitud_pqrs=".$id." and estado_clasificacionpqrs_dvcn=1 limit 1"); 
$selectb = mysql_query($queryb, $conexion) or die(mysql_error());
$rowb = mysql_fetch_assoc($selectb);
$totalRowsb = mysql_num_rows($selectb);
...
$supervigilado=$rowb['id_vigilado'];
		...
		if ( 5!=$estado_solicitud_pqrs && ((0<$sigrupo && 45==$_SESSION['snr_grupo_area'] && (1==$_SESSION['snr_grupo_cargo'] or 1==$_SESSION['snr_lider_pqrs']) ) or 1==$_SESSION['rol'])) { 
			echo ' <a style="color:#ff0000;cursor: pointer" title="Borrar" name="clasificacionpqrs_dvcn" id="'.$rowb['id_clasificacionpqrs_dvcn'].'" class="borrar_f"><span class="glyphicon glyphicon-trash"></span></a>';
			} else {}
			echo '<br>';
            echo quees('tipologiapqrs_notaria', $rowb['id_tipologiapqrs_notaria']);
		echo '<br>';
	    echo $rowb['fecha_clasificacionpqrs_dvcn'];
...
if (5!=$estado_solicitud_pqrs && ((0<$sigrupo && 45==$_SESSION['snr_grupo_area']) or 1==$_SESSION['rol'])) { ?>
...
if ((0<$sigrupo && 45==$_SESSION['snr_grupo_area'] && 1==$_SESSION['snr_grupo_cargo']) or 1==$_SESSION['rol']) {
...
<?php if (1==$_SESSION['rol'] && 0!=$supervigilado) { ?>
...
<div class="modal-header"> 
<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Cerrar</span></button>
<h4 class="modal-title" id="myModalLabel"><b>ENVIO A CONTROL DISCIPLINARIO:</b> <span class="licenciac" ></span></h4>
</div> 
<div id="nuevaAventura" class="modal-body"> 
<form action="" method="POST" name="form1F545455345345GDG">
...
<div class="form-group text-left"> 
<!--<label  class="control-label">RESPUESTA AL CIUDADANO:</label>--> <!--respuesta_pre_ciudadano_nueva-->
<textarea spellcheck="true" lang="es" class="form-control" name="respuesta_control_ciudadano_nueva" id="texto_control_ciudadano">
...
?>
<br><br>
Referencia: Envio a Control disciplinario - radicado <?php echo $radicado; ?>
 <!--de fecha-->
 <?php 
//echo date("Y-m-d",strtotime($fecha_radicado.""));?>
<br><br>
...
<br><br>
<p align="justify" style="text-align: justify; text-justify: inter-word;">
De manera comedida le informo que hemos recibido su escrito radicado bajo el N° <?php echo $radicado; ?>, en el cual pone de presente unos hechos que podrían comprometer la presunta responsabilidad de la notaria <?php echo quees('notaria', $supervigilado); ?>.
<br><br>
Teniendo en cuenta que la Delegada para el Notariado tiene a su cargo las funciones de orientación, inspección, vigilancia y control respecto de los notarios, se ha dispuesto el traslado interno del asunto al Proceso de Control de la Gestión Notarial a efectos de que se determine, dentro del marco de una actuación disciplinaria, si los hechos ameritan el inicio de una indagación preliminar, una apertura de investigación, o una decisión inhibitoria.
<br><br>
En consecuencia, a partir de la fecha del traslado, el asunto se rige por los términos y regulaciones contenidas en la Ley 734 de 2002, de manera que usted adquiere la calidad de quejoso (a), conforme a lo dispuesto en el parágrafo del artículo 90 de la citada ley.
<br><br>
<br><br>
Atentamente,
<br><br>
...
$querybn = sprintf("SELECT count(id_control_disciplinario) as totreqc FROM control_disciplinario where id_solicitud_pqrs=".$id." and estado_control_disciplinario=1"); 
$selectbn = mysql_query($querybn, $conexion) or die(mysql_error());
$rowbn = mysql_fetch_assoc($selectbn);

if (0<$rowbn['totreqc']) { 
echo 'ENVIADO A CONTROL DISC.';
} else {
	
	
	
if ((0<$sigrupo && 45==$_SESSION['snr_grupo_area']) or 1==$_SESSION['rol']) { ?>
...
<span class="glyphicon glyphicon-inbox"></span>  ENVIAR A CONTROL DISC.</a>
<?php } else { echo 'CONTROL'; }
}
  ?>
</h3>
                  <div class="box-tools pull-right">       
 ...
 <div class="box-body" >
				<div  class="modal-body" style="max-height:250px;">
<?php
$query = sprintf("SELECT id_control_disciplinario, fecha_control_disciplinario FROM control_disciplinario where id_solicitud_pqrs=".$id." and estado_control_disciplinario=1 order by id_control_disciplinario"); 
$select = mysql_query($query, $conexion);
$row = mysql_fetch_assoc($select);
$totalRows = mysql_num_rows($select);
if (0<$totalRows){
do {
	echo ''.$row['fecha_control_disciplinario'].'<br><a href="control_proceso.jsp" target="_blank">';
	if ($supervigilado!=0) {
		echo 'Notaria: ';
echo quees('notaria', $supervigilado);
	} else {}
	echo '</a>';
	 } while ($row = mysql_fetch_assoc($select)); 
...
$query48 = sprintf("SELECT * FROM solicitud_pqrs, ciudadano where ciudadano.id_ciudadano=".$id_ciudadano."  and solicitud_pqrs.id_ciudadano=ciudadano.id_ciudadano and estado_solicitud_pqrs=1  order by solicitud_pqrs.id_solicitud_pqrs desc"); 
