...
$queryk = "SELECT count(id_funcionario) as totp FROM funcionario where id_vinculacion in (2, 7) and id_funcionario=".$_GET["i"].""; 
...
$queryk = "SELECT count(id_funcionario) as totp FROM funcionario where id_vinculacion in (2, 7) and  id_funcionario=".$_SESSION['snr'].""; 
$selectk = mysql_query($queryk, $conexion);
$rowpk = mysql_fetch_assoc($selectk);
$valp=$rowpk['totp'];
mysql_free_result($selectk);
...
$select = mysql_query("select count(id_candidato_votacion_personal) as totale from candidato_votacion_personal where id_funcionario=".$idf1." and estado_candidato_votacion_personal=1", $conexion);
...
$insertSQL = sprintf("INSERT INTO candidato_votacion_personal (nombre_candidato_votacion_personal, 
id_funcionario, url, estado_candidato_votacion_personal) VALUES (now(), %s, %s, %s)", 
...
$cuerpo .= "<br><br>"; 
$cuerpo .= 'Incripción: <a href="https://sisg.supernotariado.gov.co/inscripcion_votacion_personal.jsp">Ver inscripción</a>'; 
$cuerpo .= "<br><br>";  
...
<h3 class="box-title">INSCRIPCIÓN A VOTACIÓN / COMISIÓN DE PERSONAL</h3>
...
$selecti = mysql_query("select * from candidato_votacion_personal, funcionario where candidato_votacion_personal.id_funcionario=funcionario.id_funcionario and candidato_votacion_personal.id_funcionario=".$id." and estado_candidato_votacion_personal=1 limit 1", $conexion);
$rowi = mysql_fetch_assoc($selecti);
$numvota= mysql_num_rows($selecti);
if (0<$numvota) {

$idv=$rowi['id_candidato_votacion_personal'];
...
<?php 
echo 'Fecha de inscripción: '.$rowi['nombre_candidato_votacion_personal'].'<br>';
...
<label  class="control-label"><span style="color:#ff0000;">*</span> SOLICITUD: <a href="documentos/FORMATO_COMISION_PERSONAL_PERIODO_2021.docx">Descargar formato</a></label> 
