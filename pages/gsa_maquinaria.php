<?php
session_start();
include '../conf.php';
include 'listas.php';
$nump85 = privilegios(85, $_SESSION['snr']);  // COORDINADOR ASEO CAFETERIA
$nump86 = privilegios(86, $_SESSION['snr']);  // ADMINISTRADOR ASEO Y CAFETERIA

$nbom = 'JONATHAN ARDILA';

if (isset($_POST['option']) and "" != $_POST['option']) {

  $idSolicitud = $_POST['option'];

  // CONSULTA DE VARIABLES GLOBALES
  $queryes = "SELECT gsa_solicitud.fecha_inicio, gsa_solicitud.fecha_final, gsa_solicitud.estado_snr, funcionario.nombre_funcionario, gsa_proveedor.nombre_gsa_proveedor FROM gsa_solicitud
  LEFT JOIN gsa_orden
  ON gsa_solicitud.id_gsa_orden=gsa_orden.id_gsa_orden
  LEFT JOIN funcionario
  ON gsa_orden.id_funcionario=funcionario.id_funcionario
  LEFT JOIN gsa_proveedor
  ON gsa_orden.id_gsa_proveedor=gsa_proveedor.id_gsa_proveedor
  WHERE id_gsa_solicitud=$idSolicitud AND estado_gsa_solicitud=1";
  $selectes = mysql_query($queryes, $conexion);
  $rowes = mysql_fetch_assoc($selectes);
  $fechaInicioC = date_create($rowes['fecha_inicio']);
  $fechaInicial = date_format($fechaInicioC, "d/m/Y");
  $fechaFinalC = date_create($rowes['fecha_final']);
  $fechaFinal = date_format($fechaFinalC, "d/m/Y");
  $estadoSnr = $rowes['estado_snr'];
  $funcionariogsa = $rowes['nombre_funcionario'];
  $nombreProveedor = $rowes['nombre_gsa_proveedor'];


  // CONSULTA DE VARIABLES PERFIL DE MODULOS OFICINAS DE REGISTRO
  $idfun = $_SESSION['snr'];
  $consulfun = "SELECT 
  oficina_registro.id_oficina_registro,
  oficina_registro.nombre_oficina_registro,
  funcionario.nombre_funcionario
  FROM funcionario
  LEFT JOIN oficina_registro
  ON funcionario.id_oficina_registro=oficina_registro.id_oficina_registro
  WHERE funcionario.id_funcionario=$idfun AND funcionario.estado_funcionario=1";
  $selectfun = mysql_query($consulfun, $conexion);
  $rowfun = mysql_fetch_assoc($selectfun);
  if (isset($rowfun)) {
    $oficinaRegistro = $rowfun['id_oficina_registro'];
    $NombreoficinaRegistro = $rowfun['nombre_oficina_registro'];
  } else {
    $oficinaRegistro = 0;
  }


  // CONSULTA LUGAR DEL PUNTO UBICACION
  $consulpunto = "SELECT 
  punto_ubicacion.id_punto_ubicacion, 
  punto_ubicacion.nombre_punto_ubicacion,
  funcionario.nombre_funcionario
  FROM punto_ubicacion_enlace
  LEFT JOIN punto_ubicacion
  ON punto_ubicacion_enlace.id_punto_ubicacion=punto_ubicacion.id_punto_ubicacion
  LEFT JOIN funcionario
  ON punto_ubicacion_enlace.id_funcionario=funcionario.id_funcionario
  WHERE punto_ubicacion_enlace.id_funcionario=$idfun AND punto_ubicacion_enlace.estado_punto_ubicacion_enlace=1";
  $selectpunto = mysql_query($consulpunto, $conexion);
  $rowpunto = mysql_fetch_assoc($selectpunto);
  if (isset($rowpunto)) {
    $puntoUbicacion = $rowpunto['id_punto_ubicacion'];
    $NombrepuntoUbicacion = $rowpunto['nombre_punto_ubicacion'];
  } else {
    $puntoUbicacion = 0;
  }


  // BUSCA EN VERIFICA QUE LA OFICINA, PUNTO UBICACION A GSA_VERIFICA_SOLICITUD
  if (NULL != $oficinaRegistro or 0 != $oficinaRegistro) {
    $consulfun = "SELECT id_gsa_verifica_solicitud, estado_gsa_verifica_solicitud, fecha_verificado, id_funcionario, observacion, estado_observacion FROM gsa_verifica_solicitud
  WHERE id_gsa_solicitud=$idSolicitud AND id_oficina_registro=$oficinaRegistro";
  }
  if (NULL != $puntoUbicacion or 0 != $puntoUbicacion) {
    $consulfun = "SELECT id_gsa_verifica_solicitud, estado_gsa_verifica_solicitud, fecha_verificado, id_funcionario, observacion, estado_observacion FROM gsa_verifica_solicitud
  WHERE id_gsa_solicitud=$idSolicitud AND id_punto_ubicacion=$puntoUbicacion";
  }
  $selectfun = mysql_query($consulfun, $conexion);
  $rowSolicitud = mysql_fetch_assoc($selectfun);
  $idverificado = $rowSolicitud['id_gsa_verifica_solicitud'];
  $estadoverificado = $rowSolicitud['estado_gsa_verifica_solicitud'];
  $funcionarioverificado = $rowSolicitud['id_funcionario'];
  $fechaVerificadoC = date_create($rowSolicitud['fecha_verificado']);
  $fechaVerificado = date_format($fechaVerificadoC, "d/m/Y H:i:s");
  $campoObservacion = $rowSolicitud['observacion'];
  $estadoObservacion = $rowSolicitud['estado_observacion'];
  if (is_null($estadoverificado)) {
    $estadoNumVerifica = 0;
    $estadoNomVerifica = 'Pendiente';
  } elseif (1 == $estadoverificado) {
    $estadoNumVerifica = 1;
    $estadoNomVerifica = 'Verificado';
  } elseif (2 == $estadoverificado) {
    $estadoNumVerifica = 2;
    $estadoNomVerifica = 'Devolución Elementos';
  }

  if (1 == $_SESSION['rol'] or 0 < $nump86 or 0 < $nump85) {
    $privilegio = 0;
  } elseif (0 != $oficinaRegistro and 0 < privreg($oficinaRegistro, $idfun, 5, 10)) {
    $privilegio = 1;
  } elseif (0 != $puntoUbicacion) {
    $privilegio = 2;
  } ?>

  <script>
    // Funcion para no duplicar envios de formularios
    if (window.history.replaceState) {
      window.history.replaceState(null, null, window.location.href);
    }

    function cambiocantidad(tabla, cantidad_producto, id_detalle) {
      jQuery.ajax({
        type: "POST",
        url: "pages/gsa_actualizar.php",
        data: 'option=' + tabla + '-' + id_detalle + '-' + cantidad_producto,
        async: true,
        success: function(html) {}
      })
    }

    function cambioverifica(tabla, cantidad_verifica, id_detalle) {
      jQuery.ajax({
        type: "POST",
        url: "pages/gsa_actualizar.php",
        data: 'option=' + tabla + '-' + id_detalle + '-' + cantidad_verifica,
        async: true,
        success: function(html) {}
      })
    }

    function serialMaquinaria(tabla, serial_maquinaria, id_detalle) {
      jQuery.ajax({
        type: "POST",
        url: "pages/gsa_actualizar.php",
        data: 'option=' + tabla + '-' + id_detalle + '-' + serial_maquinaria,
        async: true,
        success: function(html) {}
      })
    }

    function fechaentrega(tabla, fecha_entrega, id_detalle) {
      jQuery.ajax({
        type: "POST",
        url: "pages/gsa_actualizar.php",
        data: 'option=' + tabla + '-' + id_detalle + '-' + fecha_entrega,
        async: true,
        success: function(html) {}
      })
    }

    function fechadevolucion(tabla, fecha_devolucion, id_detalle) {
      jQuery.ajax({
        type: "POST",
        url: "pages/gsa_actualizar.php",
        data: 'option=' + tabla + '-' + id_detalle + '-' + fecha_devolucion,
        async: true,
        success: function(html) {}
      })
    }

    function campoObservacion(tabla, var2, var3, var4) {
      jQuery.ajax({
        type: "POST",
        url: "pages/gsa_actualizar.php",
        data: 'option=' + tabla + '-' + var2 + '-' + var3 + '-' + var4,
        async: true,
        success: function(html) {
          location.reload()
        }
      })
    }

    function EditarcampoObservacion(tabla, var2, var3) {
      jQuery.ajax({
        type: "POST",
        url: "pages/gsa_actualizar.php",
        data: 'option=' + tabla + '-' + var2 + '-' + var3,
        async: true,
        success: function(html) {
          location.reload()
        }
      })
    }
  </script>

  <div class="box box-primary">
    <div class="row" style="margin:20px 5px;">
      <div class="table-responsive" style="height: 600px;">

        <?php if (0 == $privilegio) { ?>

          <table class="table table-striped table-bordered table-hover" id="TablaMaquinaria" cellspacing="0" width="100%">
            <thead>
              <tr style="background-color: #eee;">
                <th>Cod</th>
                <th>
                  <p style="text-align:center;"><b>Maquinaria</b></p>
                </th>
                <th>
                  <p style="text-align:center;">Valor/UND</p>
                </th>
                <th>
                  <p style="text-align:center;"><b>Cant * Fila</b></p>
                </th>
                <?php $query = "SELECT  * FROM gsa_sedes
                LEFT JOIN oficina_registro
                ON gsa_sedes.id_oficina_registro=oficina_registro.id_oficina_registro
                LEFT JOIN punto_ubicacion
                ON gsa_sedes.id_punto_ubicacion=punto_ubicacion.id_punto_ubicacion
                LEFT JOIN gsa_orden
                ON gsa_orden.id_gsa_orden=gsa_sedes.id_gsa_orden
                LEFT JOIN gsa_solicitud
                ON gsa_orden.id_gsa_orden=gsa_solicitud.id_gsa_orden
                WHERE gsa_solicitud.id_gsa_solicitud = $idSolicitud AND estado_gsa_sedes=1";
                $select = mysql_query($query, $conexion);
                $rowD = mysql_fetch_assoc($select);
                $totalRows = mysql_num_rows($select);
                $idSolicitudPrint = $rowD['id_gsa_solicitud'];
                $fechaIniSolicitudPrint = $rowD['fecha_inicio'];
                $fechaFinSolicitudPrint = $rowD['fecha_final'];
                if (0 < $totalRows) {
                  do {
                    if (isset($rowD['id_oficina_registro'])) {
                      $oficinaR = $rowD['id_oficina_registro'];
                      $query23 = "SELECT  COUNT(id_oficina_registro) AS conta FROM gsa_solicitud
                      LEFT JOIN gsa_solicitud_maquinaria
                      ON gsa_solicitud.id_gsa_solicitud = gsa_solicitud_maquinaria.id_solicitud
                      WHERE gsa_solicitud.id_gsa_solicitud = $idSolicitud AND gsa_solicitud_maquinaria.id_oficina_registro = $oficinaR";
                      $select23 = mysql_query($query23, $conexion);
                      $rowCount = mysql_fetch_assoc($select23);
                      if ($rowCount['conta'] > 0) {
                        echo '<th style="width:200px;">
                            <p style="font-size:9px; text-align:center;">
                            Sede ' . $rowD['num_sede'] . ' ' . $rowD['id_oficina_registro'] . ' ' . $rowD['nombre_oficina_registro'] . '
                            </p>
                          </th>';
                        if (1 == $estadoSnr) { ?>
                          <th>
                            <p style="font-size:9px; text-align:center;">
                              SubTotal
                            </p>
                          </th><?php
                              } // FINAL IF ESTADOSNR
                            } // FINAL DE CONTA
                          } // FINAL DE IF OFICINA REGISTRO
                          if (isset($rowD['id_punto_ubicacion'])) {
                            $puntoUbicacion = $rowD['id_punto_ubicacion'];
                            $query233 = "SELECT  COUNT(id_punto_ubicacion) AS conta FROM gsa_solicitud
                            LEFT JOIN gsa_solicitud_maquinaria
                            ON gsa_solicitud.id_gsa_solicitud = gsa_solicitud_maquinaria.id_solicitud
                            WHERE gsa_solicitud.id_gsa_solicitud = $idSolicitud AND gsa_solicitud_maquinaria.id_punto_ubicacion = $puntoUbicacion";
                            $select233 = mysql_query($query233, $conexion);
                            $rowCount3 = mysql_fetch_assoc($select233);
                            if ($rowCount3['conta'] > 0) {
                              echo '<th style="width:200px;">
                                <p style="font-size:9px; text-align:center;">
                                Sede ' . $rowD['num_sede'] . ' ' . $rowD['id_punto_ubicacion'] . ' ' . $rowD['nombre_punto_ubicacion'] . '
                                </p>
                              </th>';
                              if (1 == $estadoSnr) { ?>
                          <th>
                            <p style="font-size:9px; text-align:center;">
                              SubTotal
                            </p>
                          </th><?php
                              } // FINAL IF ESTADOSNR
                            } // FINAL DE CONTA
                          } // FINAL DE IF PUNTO UBICACION
                        } while ($rowD = mysql_fetch_assoc($select));
                        mysql_free_result($select);
                      } ?>
              </tr>
            </thead>
            <tbody>
              <?php
              $totalFacturacionExcel = 0;
              $query = "SELECT * FROM gsa_solicitud
              LEFT JOIN gsa_orden
              ON gsa_solicitud.id_gsa_orden=gsa_orden.id_gsa_orden
              LEFT JOIN gsa_maquinaria
              ON gsa_orden.id_gsa_orden=gsa_maquinaria.id_gsa_orden
              WHERE gsa_solicitud.id_gsa_solicitud = $idSolicitud AND gsa_maquinaria.maquinaria_estado=1 AND gsa_maquinaria.estado_gsa_maquinaria=1";
              $select = mysql_query($query, $conexion);
              $rowD1 = mysql_fetch_assoc($select);
              $totalRows = mysql_num_rows($select);
              if (0 < $totalRows) {
                do {
                  echo '<tr>
                      <td>' . $rowD1['numero_maquinaria'] . '</td>
                      <td> <p style="font-size:11px;">' . $rowD1['bien_maquinaria'] . '</p></td>
                      <td> <p style="font-size:11px;">' . $rowD1['precio_unitario'] . '</p></td>';

                  $idproducto = $rowD1['id_gsa_maquinaria'];
                  $querycantidad = "SELECT SUM(cantidad_producto) AS sumaCantidad FROM gsa_solicitud_maquinaria 
                  LEFT JOIN gsa_solicitud
                  ON gsa_solicitud.id_gsa_solicitud=gsa_solicitud_maquinaria.id_solicitud
                  WHERE gsa_solicitud_maquinaria.id_solicitud=$idSolicitud AND gsa_solicitud_maquinaria.id_gsa_maquinaria=$idproducto";
                  $selectcantidad = mysql_query($querycantidad, $conexion);
                  $rowCantidad = mysql_fetch_assoc($selectcantidad);
                  echo ' <td>' . $rowCantidad['sumaCantidad'] . '</td>';


                  $querydetalle = "SELECT * FROM gsa_solicitud_maquinaria
                  LEFT JOIN gsa_maquinaria
                  ON gsa_solicitud_maquinaria.id_gsa_maquinaria = gsa_maquinaria.id_gsa_maquinaria
                  WHERE gsa_solicitud_maquinaria.id_gsa_maquinaria=$idproducto AND  gsa_solicitud_maquinaria.id_solicitud=$idSolicitud AND gsa_solicitud_maquinaria.estado_gsa_solicitud_maquinaria=1";
                  $selectdetalle = mysql_query($querydetalle, $conexion);
                  $rowdetalle1 = mysql_fetch_assoc($selectdetalle);
                  $detalleRows = mysql_num_rows($selectdetalle);
                  if (0 < $detalleRows) {
                    do {
                      if (0 == $estadoSnr) { ?>
                        <td style="text-align: center;">
                          <input style="width:50px; text-align: center; border-radius:0px; border:solid 1px #999;" type="number" value="<?php echo $rowdetalle1['cantidad_producto']; ?>" onchange="cambiocantidad('cantidad_maquinaria', this.value, <?php echo $rowdetalle1['id_gsa_solicitud_maquinaria']; ?>);" />
                          <?php
                          $cantidadPrecio = $rowdetalle1['cantidad_producto'] * $rowdetalle1['precio_unitario'];
                          $totalFacturacionExcel += $cantidadPrecio;
                          ?>
                        </td>
                      <?php
                        $botones = "";
                      } elseif (1 == $estadoSnr) { ?>
                        <td style="text-align: center;">
                          <?php echo $rowdetalle1['cantidad_producto']; ?>
                        </td>
                        <td style="text-align: center;">
                          <?php
                          $cantidadPrecio = $rowdetalle1['cantidad_producto'] * $rowdetalle1['precio_unitario'];
                          $totalFacturacionExcel += $cantidadPrecio;
                          echo $cantidadPrecio;
                          ?>
                        </td><?php
                              $botones = "
                          {
                            extend: 'excelHtml5',
                            text: 'Excel',
                            title: 'REPORTE  SOLICITUD NUMERO " . $idSolicitudPrint . " DEL PERIODO DESDE " . $fechaIniSolicitudPrint . " HASTA " . $fechaFinSolicitudPrint . "',
                          },
                          ";
                            }
                          } while ($rowdetalle1 = mysql_fetch_assoc($selectdetalle));
                          mysql_free_result($selectdetalle);
                        }
                        echo '</tr>';
                      } while ($rowD1 = mysql_fetch_assoc($select));
                      mysql_free_result($select);
                    }
                              ?>
            </tbody>
            <tfoot>
              <tr>
                <th>Total</th>
                <th><?php echo number_format($totalFacturacionExcel, 2, '.', ','); ?></th>
              </tr>
            </tfoot>
            <script>
              $(document).ready(function() {
                var t = $('#TablaMaquinaria').DataTable({
                  dom: 'Bfrtip',
                  buttons: [
                    <?php echo $botones; ?>
                  ],
                  "lengthMenu": [
                    [100, 200, 300, 500],
                    [100, 200, 300, 500]
                  ],
                  "language": {
                    "url": "//cdn.datatables.net/plug-ins/1.10.22/i18n/Spanish.json"
                  },
                  "aaSorting": [
                    [0, "asc"]
                  ]
                });
              });
            </script>
          </table>

        <?php } elseif (1 == $privilegio or 2 == $privilegio) { ?>

          <table class="table table-striped table-bordered table-hover" id="cantidadMaquinarias" cellspacing="0" width="100%">
            <thead>
              <tr style="background-color: #eee;">
                <th>Cod</th>
                <th>Item</th>
                <th>Cantidad (SNR)</th>
                <th>Num Serial</th>
                <th>Cantidad Verificada</th>
                <th>Fecha Entrega</th>
                <?php if (1 == $estadoNumVerifica or 2 == $estadoNumVerifica) { ?>
                  <th title="Fecha en que entrega al Proveeedor">Fecha Devolucion</th>
                <?php } ?>
              </tr>
            </thead>
            <tbody>
              <?php
              $totalFacturacion = 0;
              $query = "SELECT * FROM gsa_solicitud
              LEFT JOIN gsa_orden
              ON gsa_solicitud.id_gsa_orden=gsa_orden.id_gsa_orden
              LEFT JOIN gsa_maquinaria
              ON gsa_orden.id_gsa_orden=gsa_maquinaria.id_gsa_orden
              WHERE gsa_solicitud.id_gsa_solicitud = $idSolicitud AND gsa_maquinaria.estado_gsa_maquinaria=1";
              $select = mysql_query($query, $conexion);
              $rowD2 = mysql_fetch_assoc($select);
              $totalRows = mysql_num_rows($select);
              if (0 < $totalRows) {
                do {
                  echo '<tr>
                        <td>' . $rowD2['numero_maquinaria'] . '</td>
                        <td> <p style="font-size:11px;">' . $rowD2['bien_maquinaria'] . '</p></td>';
                  $idproducto = $rowD2['id_gsa_maquinaria'];
                  if (1 == $privilegio) {
                    $NombreImpreso = $NombreoficinaRegistro;
                    $querydetalle = "SELECT * FROM gsa_solicitud_maquinaria
                    LEFT JOIN gsa_maquinaria
                    ON gsa_solicitud_maquinaria.id_gsa_maquinaria=gsa_maquinaria.id_gsa_maquinaria
                    WHERE gsa_solicitud_maquinaria.id_gsa_maquinaria=$idproducto AND gsa_solicitud_maquinaria.id_solicitud=$idSolicitud AND gsa_solicitud_maquinaria.id_oficina_registro=$oficinaRegistro AND estado_gsa_solicitud_maquinaria=1";
                  }
                  if (2 == $privilegio) {
                    $NombreImpreso = $NombrepuntoUbicacion;
                    $querydetalle = "SELECT * FROM gsa_solicitud_maquinaria
                    LEFT JOIN gsa_maquinaria
                    ON gsa_solicitud_maquinaria.id_gsa_maquinaria=gsa_maquinaria.id_gsa_maquinaria
                    WHERE gsa_solicitud_maquinaria.id_gsa_maquinaria=$idproducto AND gsa_solicitud_maquinaria.id_solicitud=$idSolicitud AND gsa_solicitud_maquinaria.id_punto_ubicacion=$puntoUbicacion AND estado_gsa_solicitud_maquinaria=1";
                  }
                  $selectdetalle = mysql_query($querydetalle, $conexion);
                  $rowdetalle2 = mysql_fetch_assoc($selectdetalle);
                  $detalleRows = mysql_num_rows($selectdetalle);
                  if (0 < $detalleRows) {
                    do {
                      echo '<td style="text-align: center;">' . $rowdetalle2['cantidad_producto'] . '</td>';
                      if (0 === $estadoNumVerifica) { ?>
                        <td style="text-align: center;">
                          <input style="width:150px; text-align: center; border-radius:0px; border:solid 1px #999;" type="text" value="<?php echo $rowdetalle2['serial_maquinaria']; ?>" onchange="serialMaquinaria('serial_maquinaria', this.value, <?php echo $rowdetalle2['id_gsa_solicitud_maquinaria']; ?>);" />
                        </td>
                        <td style="text-align: center;">
                          <input style="width:50px; text-align: center; border-radius:0px; border:solid 1px #999;" type="number" value="<?php echo $rowdetalle2['cantidad_verifica']; ?>" onchange="cambioverifica('verifica_maquinaria', this.value, <?php echo $rowdetalle2['id_gsa_solicitud_maquinaria']; ?>);" />
                        </td>
                        <td>
                          <?php if (is_null($rowdetalle2['fecha_entrega'])) {
                            $finalFechaEntrega = '';
                          } else {
                            $fechaentrega = date_create($rowdetalle2['fecha_entrega']);
                            $finalFechaEntrega = date_format($fechaentrega, "d/m/Y");
                          } ?>
                          <input style="width:100px; text-align: center; border-radius:0px; border:solid 1px #999;" type="text" value="<?php echo $finalFechaEntrega; ?>" onchange="fechaentrega('fechaentregamaquina', this.value, <?php echo $rowdetalle2['id_gsa_solicitud_maquinaria']; ?>);" placeholder="dia/mes/año" pattern="[0-9/]{10}" title="Formato: dia/mes/año" />
                        </td>
                        <?php
                      }
                      if (1 === $estadoNumVerifica) {
                        echo '<td style="text-align: center;">' . $rowdetalle2['serial_maquinaria'] . '</td>';
                        echo '<td style="text-align: center;">' . $rowdetalle2['cantidad_verifica'] . '</td>';
                        echo '<td style="text-align: center;">' . $rowdetalle2['fecha_entrega'] . '</td>';
                        if (1 === $estadoNumVerifica) {
                          if (is_null($rowdetalle2['fecha_devolucion'])) {
                            $finalFechaDevolucion = '';
                          } else {
                            $fechadevolucion = date_create($rowdetalle2['fecha_devolucion']);
                            $finalFechaDevolucion = date_format($fechadevolucion, "d/m/Y");
                          } ?>
                          <td style="text-align: center;">
                            <input style="width:100px; text-align: center; border-radius:0px; border:solid 1px #999;" type="text" value="<?php echo $finalFechaDevolucion; ?>" onchange="fechadevolucion('fechadevolucionmaquina', this.value, <?php echo $rowdetalle2['id_gsa_solicitud_maquinaria']; ?>);" placeholder="dia/mes/año" pattern="[0-9/]{10}" title="Formato: dia/mes/año" />
                          </td><?php
                              }
                            }
                            if (2 === $estadoNumVerifica) {
                              echo '<td style="text-align: center;">' . $rowdetalle2['serial_maquinaria'] . '</td>';
                              echo '<td style="text-align: center;">' . $rowdetalle2['cantidad_verifica'] . '</td>';
                              echo '<td style="text-align: center;">' . $rowdetalle2['fecha_entrega'] . '</td>';
                              echo '<td style="text-align: center;">' . $rowdetalle2['fecha_devolucion'] . '</td>';
                            }
                          } while ($rowdetalle2 = mysql_fetch_assoc($selectdetalle));
                          mysql_free_result($selectdetalle);
                        }
                        echo '</tr>';
                      } while ($rowD2 = mysql_fetch_assoc($select));
                      mysql_free_result($select);
                    } ?>
            </tbody>
            <?php if (0 === $estadoNumVerifica) {
            }
            if (1 === $estadoNumVerifica) { ?>
              <tfoot>
                <tr>
                  <th>Observación</th>
                  <?php if (0 == $estadoObservacion) { ?>
                    <td><textarea style="min-height: 8px; width: 100%;" onchange="campoObservacion('Observacion', this.value, <?php echo $idverificado; ?>, 1);"><?php echo $campoObservacion; ?></textarea></td>
                  <?php }
                  if (1 == $estadoObservacion) { ?>
                    <td><?php echo $campoObservacion; ?> <span class="btn btn-warning btn-xs pull-right" onclick="EditarcampoObservacion('EditarObservacion', <?php echo $idverificado; ?>, 0);">Editar</span></td>
                  <?php } ?>
                </tr>
              </tfoot>
            <?php } ?>
            <script>
              if (0 === <?php echo $estadoNumVerifica; ?>) {
                $(document).ready(function() {
                  $('#cantidadMaquinarias').DataTable({
                    "lengthMenu": [
                      [50, 100, 200, 300, 500],
                      [50, 100, 200, 300, 500]
                    ],
                    "language": {
                      "url": "//cdn.datatables.net/plug-ins/1.10.22/i18n/Spanish.json"
                    },
                    "aaSorting": [
                      [0, "asc"]
                    ]
                  });
                });
              }
            </script>
            <script>
              if (1 === <?php echo $estadoNumVerifica; ?> || 2 === <?php echo $estadoNumVerifica; ?>) {
                $(document).ready(function() {
                  $('#cantidadMaquinarias').DataTable({
                    dom: 'Bfrtip',
                    buttons: [{
                      extend: 'pdfHtml5',
                      text: 'PDF',
                      title: "MAQUINARIA Y ELEMENTOS <?php echo $NombreImpreso; ?>",
                      customize: function(doc) {
                        doc.styles.title = {
                            color: 'gray',
                            fontSize: '14',
                            alignment: 'center'
                          },
                          doc.content.splice(1, 0, {
                            columns: [{
                              margin: 12,
                              alignment: 'left',
                              image: 'data:image/png;base64,',
                              width: 100,
                              height: 30
                            }, {
                              margin: [100, 10],
                              text: "Solicitud # <?php echo $idSolicitud; ?> \n Fecha Solicitud: <?php echo $fechaInicial; ?> | <?php echo $fechaFinal; ?> \n Funcionario (Nivel Central): <?php echo $funcionariogsa; ?>  \n Proveedor: <?php echo $nombreProveedor; ?> \n Estado: <?php echo $estadoNomVerifica; ?>",
                              fontSize: 9
                            }]
                          });
                      },
                      messageBottom: " \n\n\n\n\n\n\n\n\n\n\nFirma Responsable: <?php echo quees('funcionario', $funcionarioverificado) ?> \n Fecha Recibido: <?php echo $fechaVerificado; ?> \n\n\n Observación: <?php echo $campoObservacion; ?>",
                    }],
                    "lengthMenu": [
                      [50, 100, 200, 300, 500],
                      [50, 100, 200, 300, 500]
                    ],
                    "language": {
                      "url": "//cdn.datatables.net/plug-ins/1.10.22/i18n/Spanish.json"
                    },
                    "aaSorting": [
                      [0, "asc"]
                    ]
                  });
                });
              }
            </script>
          </table>

        <?php } else {
        } ?>

      </div>
    </div>
  </div>


<?php
}
