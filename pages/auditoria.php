...
if (isset($_GET['i'])) {
$number=$_GET['i'];
$query = "SELECT * FROM auditoria where url LIKE '%radicacion_curaduria&".$number.".jsp%' order by id_auditoria desc limit 1000";
} else {
...
$query = "SELECT * FROM auditoria where ".$infobusqueda." order by id_auditoria desc limit 1000";
}
...
echo '<td>'.$row['url'].'</td>';
echo '<td>'.htmlentities($row['descripcion_auditoria']).'</td>';
echo '</tr>';
